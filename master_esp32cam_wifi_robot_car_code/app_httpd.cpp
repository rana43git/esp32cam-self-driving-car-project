...
static String fromHTML = "";                 // Variable to store input from HTML
static String fromINO = "Hello from ESP32";  // Response variable
...
static esp_err_t process_handler(httpd_req_t *req) {
  ...
  if (buf_len > 1) {
    ...
    fromHTML = String(value);  // Store received value
  } else {
    httpd_resp_send_404(req);
    return ESP_FAIL;
  }

  // Prepare response using fromINO
  String response = GPSdataMain;
...
void startCameraServer() {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &process_uri);
  }
